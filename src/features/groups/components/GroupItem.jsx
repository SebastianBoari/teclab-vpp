...
const GroupItem = ({ group, renderAction }) => {
  const { group_name, category, periods, capacity, tutors, schedule, enrolled_count = 0 } = group
  ...
  return (
    <div
      className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 flex flex-col gap-4 ${isFull ? 'opacity-90' : ''}`}
    >
      <div className="flex items-start gap-4">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-2 mt-1 text-sm text-gray-500 dark:text-gray-400">
            <img src={tutorAvatar} alt="Tutor avatar" className="w-6 h-6 rounded-full" />
            <span>{tutorName}</span>
          </div>
        </div>
      ...
      <div className="mt-2 space-y-2 text-sm text-gray-600 dark:text-gray-300">
        <div className="flex items-center gap-2">
          ...
          {schedule.length === 0 ? (
            ...
            <span>
              {schedule.length > 1 ? schedule[0].day + ' y ' + schedule[1].day : schedule[0].day} de{' '}
        ...
        <div className="flex items-center gap-2">
          ...
          <span>
            Desde {formatDate(periods.start_at)} hasta {formatDate(periods.end_at)}
          </span>
        </div>
      </div>
      ...
      {isFull ? (
        <button
          disabled
          className="w-full bg-gray-300 dark:bg-gray-700 text-gray-500 font-medium py-2.5 rounded-lg text-sm mt-2 cursor-not-allowed"
        >
          Grupo Completo
        </button>
      ) : (
        (renderAction?.(group) ?? null)
      )}
    </div>
  )
}
