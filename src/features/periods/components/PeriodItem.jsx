...
const PeriodItem = ({ period, onEdit, onDelete }) => {
    ...
    return (
    <div className={`flex flex-col gap-3 p-4 rounded-xl max-w-2xl min-w-2xs border transition-all ${styles.card}`}>
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
                <div className={`flex items-center justify-center rounded-lg shrink-0 size-10 ${styles.iconBox}`}>
            ...
            <div className="flex h-6 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-800 px-2">
                <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider">Finalizado</p>
            </div>
            )}
        </div>
        ...
        <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-3 mt-1">
            <div className="flex flex-col">
                ...
                <p className={`text-sm font-medium py-2 pr-2 ${isEnded ? 'text-slate-500' : 'text-slate-900 dark:text-slate-200'}`}>
