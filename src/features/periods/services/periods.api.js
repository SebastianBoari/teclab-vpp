...
export const getPeriod = async ({ id, isEnrollmentOpen } = {}) => {
  try {
    let query = supabase.from('periods').select('*').order('start_at', { ascending: false })
    ...
    if (isEnrollmentOpen !== undefined) {
      ...
      query = query.lte('enrollment_open_at', now).gte('enrollment_close_at', now)
    }
    ...
    console.error(
      `Error en getPeriod: ${error.message}. id: ${id}, isEnrollmentOpen: ${isEnrollmentOpen}`
    )
    throw error
  }
}
...
export const getPeriods = async ({ isActive } = {}) => {
  try {
    ...
    console.error(
      `Error en getPeriods: ${error.message}. isActive: ${isActive}. Detalle: ${error.message}`
    )
    throw error
  }
}
...
export const createPeriod = async (periodData) => {
  try {
    const { data, error } = await supabase.from('periods').insert(periodData).select().single()
